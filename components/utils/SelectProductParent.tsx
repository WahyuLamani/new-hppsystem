...
export default function SelectProductParent({
  value,
  onChange,
  products,
}: {
  value: (Products & { category: Categories }) | null;
  onChange: (product: Products & { category: Categories }) => void;
  ...
  const handleSelect = (product: Products & { category: Categories }) => {
    onChange(product);
    setIsOpen(false);
    setQuery("");
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-on-surface-variant font-medium">
                {value ? value.category.name : "Belum ada yang dipilih"}
              </p>
            </div>
          </div>
          <ChevronDown
